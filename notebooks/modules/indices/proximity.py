...
def graph_walk_distance(g: nx.MultiDiGraph | nx.MultiGraph, initial_node, distance:int=None, attribute_count=None, add_property=True):
    """
    BFS graph walk.
    Distance in meters.
    """
    ...
    visit_queue.put((0, {"node_id": initial_node, "distance": 0, "parent": None, "key": None}))
    ...
    while not visit_queue.empty():
        ...
        if current_parent is not None:
            visited_edges.add(f"{current_parent},{current_id},{current_edge_key}")
        ...
        for adj_node in incident:
            ...
            visit_queue.put((new_distance, {"node_id": adj_node, "distance": new_distance, "parent": current_id, "key": short_key}))

            if attribute_count is not None:
                attribute_value = g[current_id][adj_node][short_key][attribute_count]
                if attribute_value is not None:
                    attribute_set = attribute_set.union(attribute_value)

    if add_property:
        g._node[initial_node][f"{attribute_count}_count"] = len(attribute_set)

    if attribute_count is not None: 
        return visited_edges, attribute_set
    else: 
        return visited_edges

def graph_walk_time(g: nx.MultiDiGraph | nx.MultiGraph, initial_node, time=None, attribute_count=None, add_property=True):
    """
    BFS graph walk.
    Time in minutes
    """
    ...
    visit_queue = PriorityQueue()
    visit_queue.put((0, {"node_id": initial_node, "time": 0, "parent": None, "key": None}))
    ...
    while not visit_queue.empty():
        ...
        if current_parent is not None:
            ...
            if edge_hash not in visited_edges:
                ...
                if attribute_count is not None:
                    attribute_value = g[current_parent][current_id][current_edge_key][attribute_count]
                    if attribute_value is not None:
                        attribute_set = attribute_set.union(attribute_value)
        ...
        for adj_node in incident:
            ...
            new_time = current_time + short_time # meters
            # ordered queue, so the closes element is visited first
            visit_queue.put((new_time, {"node_id": adj_node, "time": new_time, "parent": current_id, "key": short_key}))
...
def __process_nodes_time(node_list, graph, walk_time:int=15):
    print(f'processing {len(node_list)} nodes')
    total = 0
    for node in node_list:
        initial = node
        visited_edges, am_count = graph_walk_time(graph, initial, walk_time, attribute_count="amenities")
        total += len(am_count)
    return total

def __process_nodes_distance(node_list, graph, walk_distance:int=400):
    print(f'processing {len(node_list)} nodes')
    total = 0
    for node in node_list:
        initial = node
        visited_edges, am_count = graph_walk_distance(graph, initial, walk_distance, attribute_count="amenities")
        total += len(am_count)
    return total
...
def proximity_to_pois(g: nx.MultiDiGraph | nx.MultiGraph, walk_time:int=15, parallelize:bool=True, max_partition_size:int=10000):
    all_nodes = g.nodes()
    ...
    while not visit_queue.empty():
        ...
        if current_parent is not None:
            ...
            if edge_hash not in visited_edges:
                ...
                if station_count > 0:
                    graph._node[initial_node]["has_public_transport"] = True
                    return True
...
def proximity_to_public_transport(graph:nx.MultiDiGraph | nx.MultiGraph, public_transport_g:nx.MultiDiGraph, distance:int):
    ...
    nodes_with_station = 0
    max_distance = distance #m
    for node_id in graph.nodes():
        nodes_with_station += 1 if has_station_in_distance(graph, node_id, max_distance) else 0

    percentage_station = nodes_with_station / number_of_nodes
